...
{
  FLOATTYPE              x;            /* X coordinate component            */
  FLOATTYPE              y;            /* Y coordinate component            */
...
typedef struct edge_shape
{
  ...
  FLOATTYPE              xb;           /* Scanbeam bottom x coordinate      */
  FLOATTYPE              xt;           /* Scanbeam top x coordinate         */
  FLOATTYPE              dx;           /* Change in x for a unit y increase */
...
{
  FLOATTYPE              y;            /* Y coordinate at local minimum     */
...
{
  FLOATTYPE              y;            /* Scanbeam node y value             */
...
{
  FLOATTYPE             xmin;          /* Minimum x coordinate              */
  FLOATTYPE             ymin;          /* Minimum y coordinate              */
  FLOATTYPE             xmax;          /* Maximum x coordinate              */
  FLOATTYPE             ymax;          /* Maximum y coordinate              */
} bbox;
...
static edge_node **bound_list(lmt_node **lmt, FLOATTYPE y)
{
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, FLOATTYPE y)
{
  if (!*sbtree)
  {
...
static void build_sbt(int *entries, FLOATTYPE *sbt, sb_tree *sbtree)
{
  if (sbtree->less)
...
static void build_intersection_table(it_node **it, edge_node *aet, FLOATTYPE dy)
{
  st_node   *st, *stp;
  edge_node *edge;
...
static void add_left(polygon_node *p, FLOATTYPE x, FLOATTYPE y)
{
  vertex_node *nv;
...
static void add_right(polygon_node *p, FLOATTYPE x, FLOATTYPE y)
{
  vertex_node *nv;
...
static void add_vertex(vertex_node **t, FLOATTYPE x, FLOATTYPE y)
{
  if (!(*t))
  {
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    box[c].xmin= FLTYPE_MAX;
    box[c].ymin= FLTYPE_MAX;
    box[c].xmax= -FLTYPE_MAX;
    box[c].ymax= -FLTYPE_MAX;
...
void  gpc_del_contour(gpc_polygon *p,int n)
{
  ...
  for (c= n+1; c < p->num_contours; c++)
  {
    extended_hole[c-1]= p->hole[c];
    extended_contour[c-1]= p->contour[c];
  }
                      ...
                      gpc_polygon *result)
{
  ...
  FLOATTYPE        *sbt= NULL, xb, px, yb, yt, dy, ix, iy;
  int i;
//  _set_sbh_threshold(500);//������ ��� ������ � IDE Delphi ����������� �������� ������ � ����
...
//  for(i=0;i<subj->num_contours;i++) {if (subj->hole[i]==1) subj->hole[i]=0; else subj->hole[i]=1;}
//  for(i=0;i<clip->num_contours;i++) {if (clip->hole[i]==1) clip->hole[i]=0; else clip->hole[i]=1;}
  ...
  MALLOC(sbt, sbt_entries * sizeof(FLOATTYPE), "sbt creation");
  ...
  FREE(s_heap);
  FREE(sbt);
//  for(i=0;i<result->num_contours;i++) {if (result->hole[i]==1) result->hole[i]=0; else result->hole[i]=1;}
}
                       ...
                       gpc_tristrip *result)
{
  ...
  int            vclass, bl, br, tl, tr;
  FLOATTYPE        *sbt= NULL, xb, px, nx, yb, yt, dy, ix, iy;
